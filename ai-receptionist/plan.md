# WhatsApp AI Receptionist — Plan (Ollama + OpenAI SDK + Parallel Agents)

**Project:** `ai-receptionist`  
**Repo:** `whatsmeow/`  
**Last updated:** May 2026  
**Direction:** Keep `whatsmeow` as the connection layer; add OpenAI Go SDK as an optional “brain”; implement a planner → parallel agents → collation workflow optimized for low perceived latency in Singapore.

---

## 1) Why this plan

The current app is already a solid MVP: WhatsApp connectivity + debounced replies + SQLite memory + lead qualification using **Ollama Cloud**. The next step is to:

- Support **OpenAI** as an additional provider (without losing Ollama).
- Add an **agentic workflow** that can split work into parallel sub-tasks (planner + agents) for speed and reliability, as described by Vignesh.

This remains a **single Go binary** running 24/7 on a VPS (or always-on machine). Not a SaaS.
...
- **WhatsApp**: `whatsmeow` linked-device login, inbound filter, outbound send, per-chat mutex.
- **AI**: Ollama Cloud `/api/chat` via `OLLAMA_API_KEY` (default `gemma4:31b-cloud` model in `config.json`).
- **Memory**: `database.db` stores `contacts` + `messages`; last ~10 turns used.
- **Receptionist mode**: structured JSON output (`reply`, `lead_updates`, `qualified`, `summary`).
- **Personal mode**: plain text replies.
- **Safety**: post-sanitize reply to avoid over-promises.
- **Ops**: errors log, fallback WhatsApp message on AI failure.

### 2.2 Where AI is called today

Inbound → debounce → `Handler.process()` → `ai.Complete(ctx, msgs, jsonMode)` → send reply.
...
- Support **two AI providers**:
  ...
  - `openai` (new)
- Switching should be **config/env-only** (no code edits to change providers).
- Structured receptionist JSON must remain compatible with the existing parser (`ParseStructuredResponse`).

### 3.2 Fast “planner + agents” execution framework (v1)

Implement Vignesh’s architecture as a first-class flow:

1) Planner produces a plan (small JSON)  
2) Run agent tasks **in parallel** (goroutines)  
3) Collate results  
4) Send final WhatsApp reply

In v1, “tools” are **stubbed** (no real calendar booking yet), but the framework is real.

### 3.3 Singapore-first latency (UX)

Optimize for low perceived latency for users in Singapore:

- typing indicator starts quickly
- if AI work exceeds a threshold, send a short “ack” message
- firm timeouts + graceful fallback
- avoid repeated TLS handshakes (client reuse)

---

## 4) Provider strategy (Ollama + OpenAI)

### 4.1 Keep Ollama as baseline

Preserve the existing Ollama client behavior and config surface (`OLLAMA_API_KEY`, `OLLAMA_API_URL`, `model` default).

### 4.2 Add OpenAI using the official Go SDK

- Use the official SDK: `github.com/openai/openai-go`
- Prefer the **Responses API** (the SDK’s primary API surface).
- Prefer **streaming** to reduce time-to-first-token (TTFT).

Singapore specifics:

- Default OpenAI base URL: `https://sg.api.openai.com`
- Note: OpenAI docs indicate Singapore supports **regional storage** but not necessarily **regional processing**, so total latency can still be dominated by where compute runs. We optimize what we control (connection reuse, streaming, smaller models, shorter prompts).

### 4.3 Provider selection

Add provider selection (config or env), e.g.:
...
- `OPENAI_API_KEY`
- `OPENAI_BASE_URL` (default `https://sg.api.openai.com`)
- `OPENAI_MODEL` (default tuned for speed)

---

## 5) Agentic workflow (planner → parallel agents → collation)

### 5.1 High-level flow

```mermaid
graph TD
  inbound[WhatsAppInbound] --> debounce[Debounce]
  debounce --> planner[PlannerCall]
  planner -->|needsInfo| askUser[AskUser]
  askUser --> planner
  planner -->|agentTasks| runAgents[RunAgentsParallel]
  runAgents --> collate[CollationCall]
  collate --> sendReply[SendReply]
```

### 5.2 Planner output contract (JSON)

Planner returns:

- `goal`
- `agents[]`: each `{name, tool, input, expected_output}`
- `questions[]`: if user info is required before proceeding
- `final_response_mode`: `structured` (receptionist) or `text` (personal)

Constraints:

- hard cap on number of agents (e.g. 3–5)
- each agent must be safe/idempotent in v1 (no external side effects)

### 5.3 Stub tools for v1

Implement tool handlers that return deterministic “stub” payloads:
...
In v2, these stubs can be replaced with real integrations (Google Calendar, etc.).

### 5.4 State handling (so questions work)

Store “pending plan/questions” per conversation in SQLite so a multi-turn task can continue after the user answers.

---

## 6) Latency + reliability tactics (to keep it fast)

- **Reuse clients**: OpenAI client and HTTP transports must be long-lived.
- **Streaming**: use streaming to reduce perceived latency; stop once we have a complete structured JSON object.
- **Timeouts**:
  - short planner timeout (fast decision)
  - medium agent timeout
  - strict overall ceiling with fallback reply
- **UX**:
  - start typing presence immediately when planner starts
  - if >2–3s, send a short ack (“Got it — checking now.”)
- **Prompt control**:
  - keep history limit small
  - avoid sending unnecessary context
- **Retries**: only for transient failures (429/5xx), with backoff, never infinite.

---

## 7) Engineering plan (concrete steps)

### Step A — Introduce a provider interface

- Define an internal interface for “complete” calls.
- Refactor the receptionist handler to depend on the interface (not the concrete Ollama client).

### Step B — Add OpenAI provider implementation

- Implement `Complete()` using OpenAI Go SDK (Responses API).
- Ensure receptionist mode still returns valid JSON compatible with `ParseStructuredResponse`.

### Step C — Add provider selection

- Choose provider by config/env.
- Add Singapore base URL defaults for OpenAI.

### Step D — Add planner + agents executor (stub tools)

- Add planner prompt + JSON parsing.
- Run agent tasks concurrently with goroutines.
- Add collation step to produce final reply.
- Persist plan/questions state in SQLite.

### Step E — Validation

- Unit tests: plan parsing + agent fanout + collation input building.
- Manual WhatsApp tests: success path, slow path (ack), provider fallback.

---

## 8) Out of scope (this iteration)

- Real calendar booking integration (Google Calendar, ICS invites)
- Payments
- CRM/dashboard
- Multi-tenant SaaS
...
- **Perceived latency**: typing indicator almost immediately after debounce.
- **Median reply**: <5s for typical simple messages (model + network dependent).
- **Reliability**: always send *some* response (fallback) even if AI fails.
- **Agentic usefulness**: planner produces sensible sub-tasks; parallel execution reduces overall time vs sequential.

---

*End of plan. Update this file when the OpenAI provider and agentic executor land, and when stub tools are replaced with real integrations.*
